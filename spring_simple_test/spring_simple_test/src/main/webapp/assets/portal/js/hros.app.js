				...
				for(var j = 1; j <= HROS.CONFIG.deskLength; j++){
					...
					if(sc['desk' + j] != ''){
						$(sc['desk' + j]).each(function(i){
							var tmpImgSrc = '';
							if(this.resourceDTO == null || this.resourceDTO.fileId == null || this.resourceDTO.fileId == ''){
								tmpImgSrc = Util.fullUrl(HROS.CONFIG.basicStaticUrl,this.icon);
							}else{
								tmpImgSrc = Util.fullUrl(HROS.CONFIG.basicDynamicUrl,'portal/portalDownload.action?resId='+this.realid);
							}
